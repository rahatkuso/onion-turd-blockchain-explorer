...
#ifndef CROWTUR_RPCCALLS_H
#define CROWTUR_RPCCALLS_H
...
{
// declare struct in turd's cryptonote namespace.
// turd should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Turd daemon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Turd daemon at "
                 << daemon_url << endl;
            return false;
        }
