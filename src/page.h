...
#ifndef CROWTUR_PAGE_H
#define CROWTUR_PAGE_H



#include "mstch/mstch.hpp"

#include "turd_headers.h"
...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        const double& lok_amount = TUR_AMOUNT(fee);
...
show_service_node(const std::string &service_node_pubkey)
{
    ...
    else
    {
      ...
      snprintf(buf, sizeof(buf),
          "This service node is awaiting to be registered and has: %s turd to be contributed remaining",
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Turd address not provided!");
    }
    ...
    // parse string representing given turd address
    ...
    if (!lokeg::parse_str_address(lok_address_str,  address_info, nettype))
    {
        ...
        return string("Cant parse Turd address: " + lok_address_str);
    }
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_turd_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
search(string search_text)
{
    ...
    // check if turd address is given based on its length
    ...
    // check if integrated turd address is given based on its length
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Turd address not provided";
        return j_response;
    }
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse Turd address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_turd_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get Turd network info";
        return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"turd_version_full"   , string {TURD_VERSION_FULL}},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = TUR_AMOUNT(txd.fee) / tx_size;
...
bool
get_turd_network_info(json& j_info)
{
