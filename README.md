# Onion Turd Blockchain Explorer

Currently available Turd blockchain explorers have several limitations which are of 
special importance to privacy-oriented users:
 ...
 - do not support Turd testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Turd Blockchain Explorer. The example not only shows how to use 
Turd C++ libraries, but also demonstrates how to use:
...
## Onion Turd Blockchain Explorer features
 ...
 - showing public components of Turd addresses,
 - decoding which outputs and mixins belong to the given Turd address and viewkey,
 - can prove that you send Turd to someone,
 - detailed information about mixins, such as, mixins' age, timescale, mixin of mixins,
 - showing number of amount output indices,
 - support Turd testnet, stagenet network,
...
##### Compile latest Turd master version

Download and compile recent Turd into your home folder:

```bash
# first install Turd dependecines
sudo apt update
...
git clone --recursive https://github.com/turd-project/turd
...
Once the Turd is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/turd
cd ~

# download the source code
git clone https://github.com/turd-project/onion-turd-blockchain-explorer.git

# enter the downloaded sourced code folder
cd onion-turd-blockchain-explorer
...
# alternatively can use: cmake -DTURD_DIR=/path/to/turd ..
# if turd is not in ~/turd
#
...
By default it will look for blockchain in its default location i.e., `~/.turd/lmdb`.
...
```
lokblocks, Onion Turd Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Turd total emission monitoring
                                        thread
...
alias lokblocksmainnet='~/onion-turd-blockchain-explorer/build/lokblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias lokblockstestnet='~/onion-turd-blockchain-explorer/build/lokblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Turd emission 

Obtaining current Turd emission amount is not straight forward. Thus, by default it is 
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.turd/lmdb/emission_amount.txt`. For testnet or stagenet networks, 
 it is `~/.turd/testnet/lmdb/emission_amount.txt` or `~/.turd/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.turd/lmdb/emission_amount.txt`
...
```
Turd emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Turd daemon's  `print_coinbase_tx_sum` command. 
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_turd",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "turd_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Turd.
...
Return raw json tx data, as represented in Turd.
